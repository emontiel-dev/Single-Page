...
import { openEnvioModal } from './envio.modal.js'; // Importamos la función para abrir el modal de envío
import { openCargoModal } from './cargo.modal.js'; // Importamos la función para abrir el modal de cargo
import { openDescuentoModal } from './descuento.modal.js'; // Importamos la función para abrir el modal de descuento
import { openPaModal } from './pa.modal.js'; // <-- Importamos la función para abrir el modal de PA
import { openClienteModal } from '../../clientes/logica/cliente.modal.js'; // <-- IMPORTAR MODAL DE CLIENTE


export async function renderCatalogo(container) {
    try {
        // Cargar el contenido de la plantilla HTML del catálogo
        const response = await fetch('src/modulos/venta/views/catalogo.html');
        if (!response.ok) {
