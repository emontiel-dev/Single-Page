...
import { addItemToCart } from './carrito.js'; // Importamos la función para añadir al carrito
...
export async function openCatalogoModal(productId) {
    ...
    const response = await fetch('src/modulos/venta/views/catalogo.modal.html');
    if (!response.ok) {
