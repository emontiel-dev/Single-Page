...
import { cartItems, updateCarritoDisplay } from './carrito.js';
import { resetModalLogic as resetCatalogoModalLogic } from './modal.logica.js';
...
export async function openEnvioModal() {
    ...
    if (existingModal) {
        ...
        // TODO: Añadir limpieza para otros modales si existen (descuento, cargo, pa)
    }

    // Cargar la plantilla HTML del modal
    const response = await fetch('src/modulos/venta/views/envio.modal.html');
    if (!response.ok) {
