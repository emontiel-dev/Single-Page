...
import { addItemToCart, cartItems, updateCarritoDisplay } from './carrito.js'; // Importar funciones del carrito
...
export async function openDescuentoModal() {
    ...
    if (existingModal) {
        ...
        // TODO: Añadir limpieza para otros modales si existen (cargo, pa, envio)
    }
    ...
    try {
        const response = await fetch('src/modulos/venta/views/descuento.modal.html');
        if (!response.ok) {
