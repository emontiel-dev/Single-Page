...
export async function openCarritoVerItemsModal() {
    ...
    try {
        // Cargar la plantilla HTML del modal de recibo
        const response = await fetch('src/modulos/venta/views/carrito.ver.items.html');
        if (!response.ok) {
            console.error('Error loading receipt modal template:', response.statusText);
            return;
        }
